...
/**
 * Reads a replication slot's {@code wal_status} back, drops a slot a test class left behind, and burns WAL past
 * the container's {@code max_slot_wal_keep_size} so a checkpoint invalidates a slot - the only way a test reaches
 * a lost one, since nothing in the module can set {@code wal_status} directly.
 *
...
public class ReplicationSlots {
     ...
     * inlined rather than bound, since {@code execute} takes no parameters and every caller passes a constant.
     */
