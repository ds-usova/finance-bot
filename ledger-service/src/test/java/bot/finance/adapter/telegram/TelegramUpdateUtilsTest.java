...
class TelegramUpdateUtilsTest {
    ...
    class ToIncomingMessage {
        ...
        @Test
        @DisplayName("when the update carries a chat id and non-blank text - then returns a command with the chat id as conversation id and that text")
        void whenUpdateCarriesChatIdAndNonBlankText_thenReturnsCommandWithChatIdAsConversationIdAndThatText() {
            Update update = BotUtils.parseUpdate(TelegramFixtures.textMessageUpdate(UPDATE_ID, CHAT_ID, "lunch 12 euro"));
        ...
        void whenUpdateCarriesNoUsableTextMessageInAChat_thenReturnsAnEmptyOptional(String description, String updateJson) {
        ...
        static Stream<Arguments> skippableUpdates() {
            return Stream.of(
                    ...
                    arguments("text but no chat", TelegramFixtures.textMessageUpdateWithoutChat(UPDATE_ID, "lunch 12 euro")));
        }
