...
| `MCP_JWT_KEYSTORE`          | where the MCP token signing keystore is read from            | the committed development keystore           | yes                      | no     |
| `MCP_JWT_KEYSTORE_PASSWORD` | the password for that keystore                               | *(a local placeholder)*                      | yes                      | yes    |
| `MCP_JWT_KEY_ALIAS`         | which key pair in the keystore signs and verifies MCP tokens | `mcp-signing`                                | with a supplied keystore | no     |
...
- The keystore `MCP_JWT_KEYSTORE` defaults to is committed to the repository, and its password with it. It is a
  development convenience and nothing more: a deployment supplies its own keystore and password.
