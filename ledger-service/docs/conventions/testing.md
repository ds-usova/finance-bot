...
- Every test method carries `@DisplayName` as `"when [condition] - then [outcome]"`: **one condition, one
  ...
  `DisplayNameConventionsTest` in `bot.finance.architecture` enforces both halves, so a name of 120 characters
  or more fails the run rather than waiting for review.
