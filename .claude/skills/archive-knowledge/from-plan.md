...
**The design file it links**, for the `D<n>` entries the ADR section below reads.
...
and nothing else. A plan carrying no such item yields no ADR.
...
repository already determined them, so the code and the conventions own the fact. Cite the `D` number in the
