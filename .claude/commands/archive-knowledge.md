...
its slice of the work list marked new or update, the plan path, its diff scope, the module conventions —
including what they say about writing documentation — and for each edge the counterpart, which side this service
is on, and the path of the use-case document on the other side. The whole work list determines those paths, so an agent can link a file a parallel agent is still
writing.
