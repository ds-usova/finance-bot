 ...
 * short enough to observe within a test. Publishes {@link CallerTokens}' key set and stubs the embeddings
 * endpoint before each test — the backfill fires on the class's one-second purge interval and must never meet
 * an unstubbed endpoint — and truncates {@code incoming_message} and {@code stream_entry_failure} after it so
 * one class's rows never leak into the next (@{@code recorded_expense} cascades from {@code incoming_message}).
...
public abstract class AbstractMemorySystemTest extends AbstractSystemTest {
    ...
    @BeforeEach
    void publishKeySetAndStubEmbeddings() {
