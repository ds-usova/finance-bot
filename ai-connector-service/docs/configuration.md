...
- `OPENAI_BASE_URL` is the base the chat-completions path hangs off, version segment included. One without it
  makes the provider answer not-found on every call, which reaches the caller as unavailable.
- `OPENAI_MODEL` must be able to call tools. One that cannot answers with text instead, so the turn succeeds
...
- `LEDGER_MCP_URL` is the ledger's base address; the tool path is fixed. Its default reaches a ledger on the
  same machine and nothing else, so a deployment supplies it. A wrong one fails every turn as
  [unavailable](contracts/out/ledger-mcp.md#failures).
- `MEMORY_ENABLED=false` runs the service with no database at all: no message is stored, no purge runs, and the
  caller's token reaches the ledger unread. None of the `when memory is on` variables is read.
- The [ledger's key set](contracts/out/ledger-mcp.md#the-ledgers-key-set) is read under the same address, so
  pointing the tools somewhere points the key set with them.
...
- A `MEMORY_MAX_AGE` or `MEMORY_PURGE_BATCH` that is not positive stops startup.
