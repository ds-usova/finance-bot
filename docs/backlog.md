...
| task 34   | R1 | `ledger-service`                         | `ResolveProposalsUseCaseTest` proves "already accepted" twice, once per count, instead of parameterizing it  | [findings](implemented/34-one-expense-table-with-a-status/review/findings.md)                 |
| task 34   | R2 | `ledger-service`                         | `ck_expense_pending_has_message` proved in two tests, one of them paying for a container to do it            | [findings](implemented/34-one-expense-table-with-a-status/review/findings.md)                 |
