...
The change stream carries raw row changes today: an `INSERT` on `expense_proposal`, a `DELETE` paired with an
`INSERT` by transaction id, a `category` row with no name for its parent. Every consumer has to re-derive what
the ledger meant, and backlog R3 lists what that costs the connector.
...
| The publication and the replica identity                           | [`V009`](../../ledger-service/src/main/resources/db/migration/V009__publish_ledger_changes.sql)                                                                                                                             | Rewritten in place: the outbox replaces the three tables, the identity lines go (D6, F4)                               |
...
| Accept and discard, as one statement each                          | [ADR 0012](../../ledger-service/docs/adr/0012-a-set-of-rows-moves-between-tables-in-one-statement.md), [`ExpenseProposalEntityRepository`](../../ledger-service/src/main/java/bot/finance/adapter/persistence/ExpenseProposalEntityRepository.java) | Kept to one statement; the statement learns both ids and writes the events (F6)                    |
| Every write that changes spending                                  | [`ExpenseProposalRepositoryAdapter`](../../ledger-service/src/main/java/bot/finance/adapter/persistence/ExpenseProposalRepositoryAdapter.java), [`ExpenseRepositoryAdapter`](../../ledger-service/src/main/java/bot/finance/adapter/persistence/ExpenseRepositoryAdapter.java) | Each transaction gains its outbox rows; discard gains the clock's instant (F8)              |
...
No new migration. `V009` has been applied to no database, so it is rewritten in place (D6). Renamed to say what
it now does, `V009__publish_facts_through_an_outbox.sql`:
...
The three `REPLICA IDENTITY FULL` lines go: they served deletes and updates on tables no longer captured (F4).
...
| `proposalId`                  | the proposal's row id — on a proposal event, and on `ProposalAccepted`               |
| `expenseId`                   | the expense's row id — on an expense event, and on `ProposalAccepted`                |
...
|---------------------|------------------------------------------------------------|---------------------------------------------------------------|-----------------------------------------------|
| `ProposalCreated`   | the model proposed an expense from a message               | [create an expense proposal](../../ledger-service/docs/usecases/create-an-expense-proposal.md) | the proposal, `proposalId`   |
| `ProposalRefiled`   | a pending proposal was moved to another category           | [change an expense's category](../../ledger-service/docs/usecases/browse-expenses.md), the pending arm | the proposal, `proposalId` |
| `ProposalDiscarded` | the person rejected the proposal                           | [resolve a reported proposal](../../ledger-service/docs/usecases/resolve-a-reported-proposal.md), discard | the proposal as it was, `proposalId` |
| `ProposalAccepted`  | the person confirmed it, and it is now an expense          | resolve, accept; [accept chosen proposals](../../ledger-service/docs/usecases/accept-chosen-proposals.md) | the expense, `proposalId` and `expenseId` |
| `ExpenseRecorded`   | an expense was recorded with no proposal                   | [create an expense](../../ledger-service/docs/usecases/create-an-expense.md)                    | the expense, `expenseId`     |
| `ExpenseRefiled`    | a recorded expense was moved to another category           | change an expense's category, the recorded arm                | the expense, `expenseId`                      |
...
resolved, or unknown — emits nothing (ADR 0012).
...
| accept, by message or by ids | one statement each, as ADR 0012: the delete draws the new expense id per row it returns, the insert stores it under that id, and the statement returns each moved row with both ids and its names (F6). Accept by ids still returns its message ids to the caller (F20) |
...
| [`change-capture.md`](../../ledger-service/docs/contracts/out/change-capture.md)   | The captured set is `outbox` and the heartbeat; the flow loses its lookup; the schema line names the renamed `V009` |
...
- **A1:** a proposal is published as a fact
  ...
  - Then: one entry reaches `ledger.cdc` with `type: ProposalCreated` and a body carrying `proposalId`, the
    message id, the content, and the category and grouping with their names

- **A2:** an acceptance is one fact per proposal
  - Given: three pending proposals under one message
  - When: the report is confirmed
  - Then: three `ProposalAccepted` entries follow, each with its `proposalId` and the `expenseId` of the expense
    it became, and each expense's row holds that id
...
- **D6:** Is the change a new migration, or a rewrite of `V009`?
  - Answer: A rewrite. `V009` creates the outbox and publishes it with the heartbeat; the three tables are never
    published and never carry `REPLICA IDENTITY FULL`. The file is renamed to what it now does. A developer's
    local database that applied the old `V009` fails Flyway's checksum and is rebuilt from scratch.
  - Basis: decided — the user chose editing the unapplied migration over creating a publication in one and
    altering it in the next (user, 2026-08-17). [`database.md`](../../ledger-service/docs/contracts/out/database.md)'s
    append-only rule protects a database that has run the migration, and none has.
...
| F4  | Does `REPLICA IDENTITY FULL` stay on the three tables?          | No — it made deletes legible on tables no longer captured, and it doubles what an update writes to the log            | Design 23 F3, F13                                                                                                 |
...
| F6  | How does one accept statement know both ids?                    | The delete's `RETURNING` draws `nextval` on the expense sequence per row; the insert stores under it; the statement returns the pair with the row, and the events are built from that | [ADR 0012](../../ledger-service/docs/adr/0012-a-set-of-rows-moves-between-tables-in-one-statement.md) |
| F7  | Where do the names come from?                                   | Joined from `category` and its parent inside the writing transaction; no cache, no window                             | [`ExpenseProposalEntityRepository`](../../ledger-service/src/main/java/bot/finance/adapter/persistence/ExpenseProposalEntityRepository.java), the summaries query's join |
| F8  | What is `occurredAt`?                                           | The `now` the use case stamps, microsecond-truncated as the rows are; discard gains it                                | [`ExpenseProposalRepositoryAdapter`](../../ledger-service/src/main/java/bot/finance/adapter/persistence/ExpenseProposalRepositoryAdapter.java) `accept`, `discard` |
...
| F12 | The engine's stored position across the migration?              | Nothing to keep — no database has run the old `V009`, so no slot or offset exists (D4, D6); a rebuilt local database starts fresh | [`change-capture.md`](../../ledger-service/docs/contracts/out/change-capture.md), the slot and the stored position |
...
| F20 | Both accept statements?                                         | Accept by message and accept by ids are two statements; each draws the ids and writes the events, and the second keeps returning its message ids | [`ExpenseProposalEntityRepository`](../../ledger-service/src/main/java/bot/finance/adapter/persistence/ExpenseProposalEntityRepository.java) `accept`, `acceptByIds` |
