...
R1 was taken directly after this directory was archived, without a rework of its own: the four roles plus
`TheSecurityChain` now live in `common/boot`, and `McpAdapterTest` and `CdcRecoveryEndpointTest` were narrowed
onto explicit bean lists in the same run. `review/evidence.md` therefore describes the tree at the archive
commit, not the tree those changes left.
