...
| R2 | open   | `web-app`, `ledger-service` | Show module-owned wording for a refused call, instead of the ledger's own message                    | The browser puts the ledger's `message` on screen at five call sites, and falls back to three English literals hardcoded in the pages. Those strings are composed in Java and never reach the catalogue, so a translated surface carries untranslatable text — the same reason D1 kept the currency list in the browser. `LoginPage` already does it the other way, showing `t('signIn.refused')`. Not a local edit: `ledger-api.yaml` describes `Problem.message` as "words a caller can show a person", so the schema, both browse-API contract pages and the design's F23/A13/A15 move with the code, and four things need deciding — whether the ledger's message goes to the console for diagnosis, whether any refusal keeps specificity, whether wording is per-surface or shared, and whether `Problem.message` stays in the contract for other callers |
