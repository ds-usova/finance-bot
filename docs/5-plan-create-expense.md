...
the same shape as [Initialize a new user](implemented/3-plan-initialize-a-new-user.md).
...
- **Q4:** No caller exists, so this plan has no system-test phase and the port is unreachable at runtime — the
  same position [plan 3](implemented/3-plan-initialize-a-new-user.md) ended in. Confirm nothing should reach the
  port in this plan.
- A: confirmed.
